...
import saleDetailsFakerFactory from "./saleDetailsFakerFactory";
...
const SaleDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [bookNameIds, setbookNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("bookDetails")
                        .find({ query: { $select: ["_id","bookName"] } })
                        .then((data) => {
                        setbookNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = saleDetailsFakerFactory(props.user,fakerCount,bookNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("saleDetails").create(elem));
...
export default connect(mapState, mapDispatch)(SaleDetailsFakerDialogComponent);
