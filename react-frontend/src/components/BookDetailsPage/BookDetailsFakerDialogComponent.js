...
import bookDetailsFakerFactory from "./bookDetailsFakerFactory";
...
const BookDetailsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [authorNameIds, setauthorNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("authorDetails")
                        .find({ query: { $select: ["_id","authorName"] } })
                        .then((data) => {
                        setauthorNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = bookDetailsFakerFactory(props.user,fakerCount,authorNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bookDetails").create(elem));
...
export default connect(mapState, mapDispatch)(BookDetailsFakerDialogComponent);
