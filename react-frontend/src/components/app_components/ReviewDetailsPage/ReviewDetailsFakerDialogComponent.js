...
import reviewDetailsFakerFactory from "./reviewDetailsFakerFactory";
...
const ReviewDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reviewDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("reviewDetails").create(elem));
...
export default connect(mapState, mapDispatch)(ReviewDetailsFakerDialogComponent);
