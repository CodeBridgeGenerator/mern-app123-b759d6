...
import authorDetailsFakerFactory from "./authorDetailsFakerFactory";
...
const AuthorDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = authorDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("authorDetails").create(elem));
...
export default connect(mapState, mapDispatch)(AuthorDetailsFakerDialogComponent);
